...
export default function Chat({ open, setOpen }: ChatProps) {
  ...
  const inputRef = useRef<HTMLInputElement | null>(null); // 👈 thêm ref cho input
  ...
  async function handleSubmit(e?: React.FormEvent) {
    ...
    try {
      ...
      setLoading(false);
      inputRef.current?.focus(); // 👈 giữ focus lại input sau khi gửi
    }
  }
  ...
  return (
    <div
      ref={chatRef}
      className="fixed bottom-24 right-6 w-[320px] max-h-[70vh] flex flex-col shadow-xl rounded-2xl border bg-white z-50"
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-2 border-b bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-t-2xl">
